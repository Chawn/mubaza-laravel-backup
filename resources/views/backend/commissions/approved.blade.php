                                ...
                                <div class="dataTables_length" id="example1_length">
                                    <label>แสดง
                                        <select name="paginate_select" id="paginate-select" class="form-control input-sm">
                                            <option value="{{ action('BackendController@getCommissionApproved', 10) }}{{ $keyword == '' ? '' : '?q=' . $keyword }}" {{ $paging == 10 ? 'selected' : '' }}>10</option>
                                            <option value="{{ action('BackendController@getCommissionApproved', 25) }}{{ $keyword == '' ? '' : '?q=' . $keyword }}" {{ $paging == 25 ? 'selected' : '' }}>25</option>
                                            <option value="{{ action('BackendController@getCommissionApproved', 50) }}{{ $keyword == '' ? '' : '?q=' . $keyword }}" {{ $paging == 50 ? 'selected' : '' }}>50</option>
                                            <option value="{{ action('BackendController@getCommissionApproved', 100) }}{{ $keyword == '' ? '' : '?q=' . $keyword }}" {{ $paging == 100 ? 'selected' : '' }}>100</option>
                                        </select> ต่อหน้า
                                    </label>
                                </div>
                            </div>
                            <div class="col-sm-6 col-xs-6">
                                <div id="example1_filter" class="dataTables_filter">
                                    <div class="pull-right">
                                        <label>
                                            <form action="{{ action('BackendController@getCommissionApproved') }}">
                    ...
                    <table id="table-manufacture-detail" class="table tablesorter table-bordered table-striped dataTable">
                        ...
                        @foreach($payouts as $payout)
                            <tr class="clickable-row" data-href="{{ action('BackendController@getPayoutTransferDetail', $payout->id) }}">
                                ...
                                <td><a href="{{ action('BackendController@getPayoutTransferDetail', $payout->id) }}" class="btn btn-primary"><i class="fa fa-save"></i></a></td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
